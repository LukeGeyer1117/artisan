...
class GalleryManager {
  ...
  renderGallery() {
    ...
    this.images.forEach((image, index) => {
      ...
      // Fixed gallery item with consistent sizing
      galleryItem.innerHTML = `
        <div class="gallery-item-wrapper relative rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow group" style="width: 200px; height: 200px;">
          ...
          <!-- Hover Controls -->
          <div class="absolute inset-0 flex items-start justify-end p-1 bg-black/25 opacity-0 group-hover:opacity-100 transition-opacity">
            <button 
              class="delete-button btn btn-sm btn-error btn-circle btn-outline text-white hover:bg-error hover:text-white"
              data-index="${index}" 
              title="Delete Image"
            >
              ×
          ...
          <!-- Order Badge -->
          <div class="absolute bottom-1 right-1 bg-base-100/90 text-sm text-base-content px-2 py-0.5 rounded-full shadow">
            ${index + 1}
          </div>
      ...
      // For responsive sizing, you could also use:
      // style="width: 160px; height: 160px;"  for mobile
      // style="width: 192px; height: 192px;"  for desktop
      // Or add a media query class in your CSS
